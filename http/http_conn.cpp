...
void http_conn::initmysql_result(sqlPool* pool)
{
    //从数据库连接池中取一个连接
    ...
    //在user表中检索username和passwd
    if(!con->query("SELECT username,passwd from user"))
    {
        LOG_ERROR("SELECT error:%s\n",mysql_errno(con.get()->get()));
        return;
    }

    while(con->next()){
        users[con->value(0)] = con->value(1);
    }
}

void http_conn::update_result()
{
    //在user表中检索username和passwd
    if(!sqlcon->query("SELECT username,passwd from user"))
    {
        LOG_ERROR("SELECT error:%s\n",mysql_errno(sqlcon.get()->get()));
        return;
    }

    while(sqlcon->next()){
        users[sqlcon->value(0)] = sqlcon->value(1);
    }
}
...
void http_conn::init(int sockfd,const sockaddr_in &addr, char* root,int TRIGMode,int close_log,string user,string passwd,string sqlname)
{
    ...
    //当浏览器出现连接重置时，可能时网站根目录出错或http相应格式出错或者访问的文件中的内容完全为空
    doc_root = root;
    m_TRIGMode = TRIGMode;
...
//从状态机，用于分析出一行的内容
//返回值为行的读取状态，又LINE_OK,LINE_BAD,LINE_OPEN
http_conn::LINE_STATUS http_conn::parser_line(){
    char temp;
...
//解析http的请求行，获得请求方法，目标url以及http的版本号
http_conn::HTTP_CODE http_conn::parse_request_line(char* text)
{
    //在HTTP报文中，请求行用来说明请求类型,要访问的资源以及所使用的HTTP版本，其中各个部分之间通过\t或空格分隔。
    //请求行中最先含有空格和\t任一字符的位置并返回
    //用于寻找字符串中第一次出现的位置，包括空格和制表符
    m_url = strpbrk(text," \t");
...
http_conn::HTTP_CODE http_conn::process_read()
{
    ...
    while((m_check_state == CHECK_STATE_CONTENT && line_status == LINE_OK)|| ((line_status = parser_line())== LINE_OK))
    {
        text = get_line();
...
http_conn::HTTP_CODE http_conn::do_request()
{
    ...
    if(cgi== 1 && (*(p+1) =='2' || *(p+1) =='3'))
    {
        ...
        if (*(p + 1) == '3')
        {
            ...
            //users是全局变量，一个map，在初始化时保存了全部的用户名和密码
            if(users.find(name) == users.end())
            {
                m_lock.lock();
                bool res = sqlcon->update(sql_insert);
                m_lock.unlock();
